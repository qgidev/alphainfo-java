 ...
 *
 * <p>Create one instance per application and reuse it — the underlying OkHttp client manages its
 * own connection pool. Every blocking method has an {@code *Async} counterpart that returns a
...
public class AlphaInfoClient implements AutoCloseable {
  ...
  /**
   * Release the dispatcher thread pool and evict all idle keep-alive connections held by OkHttp.
   * Safe to call more than once — the second call is a no-op because {@code
   * ExecutorService.shutdown} is idempotent and evicting an already-empty connection pool is a
   * cheap check.
   *
  ...
  public static Map<String, Object> guide(String baseUrl) {
    try (var c =
        new AlphaInfoClient("ai_noauth_placeholder_ignored", baseUrl, defaultHttpClient())) {
   ...
   *
   * <p>The {@code domain} field on the request is optional — defaults to "generic". Pass {@code
   * "auto"} to have the server infer the calibration from the signal (then read {@link
   * AnalysisResult#domainApplied} and {@link AnalysisResult#domainInference}). Or pass a specific
   * name ({@code "biomedical"}, {@code "finance"}, …); aliases like {@code "fintech"} and {@code
   * "biomed"} resolve server-side. Real typos receive HTTP 400 with a "Did you mean …?" suggestion.
   */
  ...
  /**
   * Syntactic sugar for {@link #analyze(AnalyzeRequest)} with {@code req.domain = "auto"}. The
   * server picks the best calibration from cheap signal statistics; inspect {@link
   * AnalysisResult#domainInference} on the returned value for confidence + reasoning.
   */
  ...
  private void warnIfTooShortForFingerprint(
    ...
    String qualifier =
        (baseline != null && !baseline.isEmpty()) ? "with baseline" : "without baseline";
    LOGGER.log(
        Level.WARNING,
  ...
  public Object auditList(int limit) {
    String path =
        "/v1/audit/list?limit=" + URLEncoder.encode(String.valueOf(limit), StandardCharsets.UTF_8);
    return mapResponse(get(path));
  }
  ...
  private void captureRateLimit(Response resp) {
    ...
    try {
      ...
      int r =
          Integer.parseInt(
              resp.header("X-RateLimit-Remaining") == null
                  ? "0"
